...
import { LabExamParam, LabParamCategory, MCUPackage } from '../types';

interface ImportLabParamsModalProps {
  isOpen: boolean;
  onClose: () => void;
  onImport: (params: LabExamParam[], mode: 'upsert' | 'append' | 'replace') => void;
...
export const ImportLabParamsModal: React.FC<ImportLabParamsModalProps> = ({
  isOpen,
  onClose,
  onImport,
  onDownloadTemplate,
  ...
  const [parsedRows, setParsedRows] = useState<LabExamParam[]>([]);
  ...
  const mapRowToLabParam = (row: any, index: number): LabExamParam | null => {
    ...
    const nama = getVal('Nama Pemeriksaan Lab', 'Nama Pemeriksaan', 'Nama', 'Pemeriksaan');
    if (!nama) return null;

    const kode = getVal('Kode Tes', 'Kode', 'ID') || `LAB-${String(index + 1).padStart(3, '0')}`;

    const kategoriRaw = getVal('Kategori Lab', 'Kategori', 'Bidang');
    let kategori: LabParamCategory = 'Hematologi';
    const foundCat = LAB_CATEGORIES.find(
      (c) => c.toLowerCase() === kategoriRaw.toLowerCase()
    );
    if (foundCat) {
      kategori = foundCat;
    } else {
      const partial = LAB_CATEGORIES.find(
        (c) =>
    ...
    const subKategori = getVal('Sub Kategori', 'Subkategori', 'Kelompok') || '';
    ...
    const nilaiRujukanPria = getVal('Nilai Rujukan Pria', 'Rujukan Pria', 'Nilai Pria', 'Normal Pria') || '-';
    const nilaiRujukanWanita = getVal('Nilai Rujukan Wanita', 'Rujukan Wanita', 'Nilai Wanita', 'Normal Wanita') || nilaiRujukanPria || '-';
    const satuan = getVal('Satuan', 'Unit') || '-';
    const metodeTes = getVal('Metode Tes', 'Metode', 'Alat') || 'Automated Analyzer';
  ...
  const processDataRows = (rawRows: any[]) => {
    ...
    const mapped: LabExamParam[] = [];
    rawRows.forEach((row, i) => {
      const item = mapRowToLabParam(row, i);
      if (item) mapped.push(item);
    });

    if (mapped.length === 0) {
      setErrorMsg('Tidak dapat menemukan data parameter laboratorium yang valid. Pastikan header kolom sesuai template (Nama Pemeriksaan Lab, Kode Tes, Kategori Lab, Target Kode Paket, Nilai Rujukan Pria, Nilai Rujukan Wanita, Satuan).');
      setParsedRows([]);
      return;
    }
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-extrabold text-base flex items-center gap-2">
                Import Parameter Laboratorium Klinis
              </h3>
              <p className="text-xs text-teal-200">
                Unggah spreadsheet Excel atau tempel data uji laboratorium sesuai standar tabel
              </p>
            </div>
          </div>
          <button
        ...
        <div className="p-6 overflow-y-auto space-y-5 flex-1 text-xs">
          ...
          <div className="p-3.5 bg-teal-50/80 border border-teal-200 rounded-xl flex flex-wrap items-center justify-between gap-3">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <span className="font-bold text-teal-900">Format Template Import Excel Laboratorium</span>
                <p className="text-[11px] text-teal-700">
                  Header: No, Kode Tes, Nama Pemeriksaan Lab, Kategori Lab, Sub Kategori, Target Kode Paket, Nilai Rujukan Pria, Nilai Rujukan Wanita, Satuan, Metode Tes, Status
                </p>
              </div>
            </div>
            <button
            ...
            <div>
              ...
              >
                ...
                <div className="font-bold text-slate-800 text-sm mb-1">
                  {fileName ? `File terpilih: ${fileName}` : 'Klik atau seret file Excel Lab ke sini'}
                </div>
          ...
          {activeTab === 'paste' && (
            <div className="space-y-3">
              <textarea
                ...
                placeholder="Salin baris dari Excel / Google Sheets termasuk baris header kolom lalu tempel di sini..."
                rows={5}
              ...
              <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-200 pb-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-extrabold text-slate-800">
                    Preview Data ({parsedRows.length} Parameter Lab Terdeteksi)
                  </span>
                </div>
                ...
                <table className="w-full text-left border-collapse text-[11px]">
                  <thead className="bg-slate-100 text-slate-700 font-bold sticky top-0 border-b border-slate-200">
                    <tr>
                      ...
                      <th className="py-2 px-2.5 w-24">Kode Tes</th>
                      <th className="py-2 px-2.5 min-w-[150px]">Nama Pemeriksaan Lab</th>
                      <th className="py-2 px-2.5 w-28">Kategori</th>
                      <th className="py-2 px-2.5 w-28">Target Paket</th>
                      <th className="py-2 px-2.5 w-28">Rujukan Pria</th>
                      <th className="py-2 px-2.5 w-28">Rujukan Wanita</th>
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {parsedRows.map((row, idx) => (
                      <tr key={idx} className="hover:bg-teal-50/50">
                        ...
                        <td className="py-1.5 px-2.5 text-slate-700 font-mono">{row.nilaiRujukanPria}</td>
                        <td className="py-1.5 px-2.5 text-slate-700 font-mono">{row.nilaiRujukanWanita}</td>
          ...
          <div className="text-slate-500 text-xs">
            {parsedRows.length > 0 ? (
              <span className="font-semibold text-teal-800">
                Siap mengimpor {parsedRows.length} parameter lab ke master sistem
              </span>
            ) : (
